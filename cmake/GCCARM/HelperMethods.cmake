if(NOT GCC_ARM_HELPER_METHODS_INCLUDED)
	...
	macro(ConfigureBuildFlags)
      ...
      set(LINK_SCRIPT ${VENDOR_LINK_SCRIPT})
		...
		if(NOT EXISTS "${LINK_SCRIPT}")
			if(NOT EXISTS "${CMAKE_SOURCE_DIR}/${LINK_SCRIPT}" AND NOT EXISTS "${CMAKE_BINARY_DIR}/${LINK_SCRIPT}")
				message(FATAL_ERROR "Cannot find the linker script. The path should be absolute or relative to CMAKE_SOURCE_DIR OR CMAKE_BINARY_DIR.\nCurrent path: ${LINK_SCRIPT}")
			else()
  ...
  macro(add_example_projects_path)
    DebugOutput("AddExamples: Adding subdirectories at: ${ARGV}")
    list(APPEND GCC_ARM_EXAMPLES_PATHS ${ARGV})
  endmacro()
